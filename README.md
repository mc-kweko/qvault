...
> **Note:** This repository previously contained a nested `qvault-main` folder which has been removed. The project root is now the top-level `qvault-main` directory.

## Setup

1. Copy `.env.example` to `.env.local` and fill in your Supabase keys (and any other secrets).
...
3. Run the development server:
   ...
   pnpm dev
   ```
4. Lint the code (warnings only):
   ```bash
   pnpm lint
   ```

See the `tsconfig.json` and `.eslintrc.js` for TypeScript and linting configuration.
